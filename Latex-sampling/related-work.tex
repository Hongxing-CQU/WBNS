...
not stop. Since then, many variations of dart throwing were proposed to
...
is to relax the original distribution to make it own blue noise
characteristics and improve the spectrum properties\cite{mccool:1992:hierarchical}.
However, the produced samples suffers from too much regularity which causes aliasing problem.
...
Baler et al.~\cite{balzer:2009:capacity} proposed a variant of Lloyd's method with capacity constraint(CCVT)
...
Goes et al.\cite{de:2012:blue}(BOT) thought of CCVT as an optimal transport problem in
the space of power diagrams.
In addition to Lloyd's method,
the kernel density model is also applied for optimal sampling.
In~\cite{fattal:2011:blue},
Fattal used a rdially-symmetric kernel function to produce an approximated density function.
...
Jiang et al.~\cite{jiang:2015:blue} used a kernel function to apply the smoothed particle hydrodynamics method for a variety of controllable blue noise sample patterns.
Compared with \cite{de:2012:blue},
our approach is generalized as a minimization of Wasserstein distance
...
\cite{Wang:1999:BlueNoise} studied the power spectrum characteristics of combined blue noise patterns and proposed a multi-class blue noise sampling approach on digital filter techniques.
However, the approach is constrained to uniform/regular/discrete sampling.
\cite{wei:2010:multi}, \cite{schmaltz:2012:multi} and \cite{jiang:2015:blue} proposed multi-class sampling
...
~\cite{chen:2012:variational} proposed a multi-class sampling strategy on with capacity-constrained Voronoi tessellation.
Two stage algorithm
 is applied for optimization of point distribution.
The first stage is the optimal procedure for each individual class,
and the second state is the optimal procedure for the combined class.
In our approach,
the combined class is directly thought of as a single class.
...
our approach preserved blue noise patterns in the density space other than in the spectrum space.
...
\subfigure[{ Dart throwing [Wei 2010]}]{
   \includegraphics[width=0.33\textwidth]{figure/2-weiliyi.png}
   \label{two-class-sampling-a}
}%
\hspace{0.1in}
\subfigure[{ Our algorithm:$\lambda_{1,2,3}=1/3,1/3,1/3$}]{
   \includegraphics[width=.33\textwidth]{figure/2-111.png}
   \label{two-class-sampling-a}
}\\
\subfigure[{ Our algorithm:$\lambda_{1,2,3}=1/4,1/4,2/4$}]{
   \includegraphics[width=.33\textwidth]{figure/2-112.png}
   \label{two-class-sampling-c}
}%
\hspace{0.1in}
\subfigure[{ Our algorithm:$\lambda_{1,2,3}=1/8,1/8,6/8$}]{
   \includegraphics[width=.33\textwidth]{figure/2-116.png}
}%
\caption{The comparison of Wei's algorithm and our algorithm for two-class blue noise sampling.
In our algorithm, $\lambda_1$,$\lambda_2$,$\lambda_3$ is the weighted parameters for red samples, blue samples and the combined samples.
...
For completeness, we briefly review Wasserstein barycenter next.
...
%\end{split}
\end{equation}
Wasserstein barycenter is an optimal probability measure $\mu$ to approximate probability measures ${\nu_1,...,\nu_N}$ with weight in Wasserstein space.
If $\mu$ is a discrete probability measure,
Wasserstein barycenter of probability measures ${\nu_1,...,\nu_N}$ is sampling with capacity constrain in the space $\Omega$.
Thus,
