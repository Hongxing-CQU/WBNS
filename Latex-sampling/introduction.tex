...
In fact, multi-class of samples is involved in a variety of sampling problem ,
...
digital filter techniques is applied to generate a multi class samples in ~\cite{Wang:1999:BlueNoise}.
In their methods, $2^n-1$ kinds of combined patters need to be created,
where $n$ is the number of class.
Thus, the problem tends to be more complicated with the increasing of the number of class.
...
However, it is a difficult to set the off-diagonal element of the conflict matrix
...
In the paper, we present an Wasserstein blue noise sampling algorithm by
...
In the paper, we present an adaptive multi-class blue noise sampling algorithm by
throwing samples as Wasserstein barycenter of multiple density distribution.
It computes point distributions to minimize Wasserstein distance.
%Blue noise property of every individual class and combined classes
%is preserved simultaneously.
Experiments show that our result are with high-quality blue noise spectrum for every individual class and the combined class.
