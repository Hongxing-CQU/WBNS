...
\begin{minipage}{1\textwidth}
 ...
 \begin{minipage}{1\textwidth}
  ...
  \end{minipage}
  \caption{Multi-class blue noise sampling  with different sizes on a same constant density function.
  Samples distribution with larger size remains blue noise pattern,
  and samples distribution with smaller size might deviate from blue noise pattern.
  The number of samples for each class is $62$,$250$,$1000$ (top) and $100$, $300$, $900$(bottom).
  The weighted parameters are set as $\lambda_{1,2,3,4}=1/4,1/4,1/4,1/4$.
...
sampling; especially, multi-class blue noise sampling is still a challenge problem
when the density distribution of every class is nonuniform and vary from each
...
On one hand, adaptive blue noise distribution property for every
individual class is guaranteed, as well as their combined class. On
the other hand, the efficiency of sampling is improved by applying
